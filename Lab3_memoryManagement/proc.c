...
waitpid(int pid, int *status, int options){
	...
	for(;;){
		...
		// No point waiting if we don't have any children. 
		if(!exist || curproc->killed){ // TODO does not exist or unexpected error
			release(&ptable.lock);
			return -1;
		}
...
setpriority(int pid, int prio){
	if(prio < -10 || prio > 10) return -1; // pri range : -10 ~ 10
...
scheduler(void)
{
	...
	for(;;){
		...
		sti();
		acquire(&ptable.lock);
		for(p = cursor; p < &ptable.proc[NPROC]; p++){
			if(p->state != RUNNABLE)
				continue;
		...
		if(next != 0){
			c->proc = next;
			if((cursor = next+1) >= &ptable.proc[NPROC]){
				cursor = ptable.proc;
			}
			switchuvm(next);
			next->state = RUNNING;
